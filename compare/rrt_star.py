...
def do_rrt_star(rand_xs,rand_ys,rand_widths,rand_heights):
    ...
    for param in data['parameters']:
        ...
        pause_after_each_map = param['pause_after_each_map']
    ...
    if(generate_random_map):
        for i in range(num_random_obstacles):
            map.add_obstacle(rand_xs[i],rand_ys[i],rand_widths[i],rand_heights[i],search_radius+1)
    else:
        for i in range(0,width,40):
            for j in range(0,height,40):
                map.add_obstacle(i,j,20,20,search_radius+1)
    ...
    if(final_path_only):
        map.display_converged_map(x_rand, final = True)
    else:
        map.display_converged_map(x_rand, final = False)
    
    if(pause_after_each_map):
        cv2.waitKey(0)
